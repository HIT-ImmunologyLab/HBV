...
string.to.colors1 = function (string, colors = NULL) 
{
  if (is.factor(string)) {
...
string.to.colors = function (string, colors = NULL) 
{
  ...
  for(i in 1:length(string)){
    if(string[i]=='TRUE'){
      Color_vector <- c(Color_vector,colors[2])
    }else{
      Color_vector <- c(Color_vector,colors[1])
    }
  }
  return(Color_vector)
}
...
for (k in List_target_path) {
  ...
  STAR_mapping_command = paste("STAR --runThreadN",N_thread,"--genomeDir",Index_genome,"--readFilesIn",k,"--outSAMattributes NH HI AS nM NM XS ",
                               ...
                               "--outFilterMatchNmin 35 --outFilterScoreMinOverLread 0.6 --outFilterMatchNminOverLread 0.6 --limitGenomeGenerateRAM 200000000000 --limitBAMsortRAM 1000000000000")
...
for (k in List_output_path) {  
  ...
  cat("start analysing each virus",nrow(temp_chromosome_count),"mapped virus","\n")

  if(nrow(temp_chromosome_count)==0){
    log_file <-  paste(k,"/no_mapped_virus_log.txt",sep = "")
    sink(log_file)
    print('0 virus was mapped to this sample!')
    sink()
    next;
  }

  QC_result = foreach(i=rownames(temp_chromosome_count),.combine = rbind,.packages = c("GenomicAlignments","ShortRead")) %dopar% {
    cat("start analysing",i,"\n")
   ...
   Reads_quality_try <- try(as(Reads_quality,"IntegerList"),silent=TRUE)
    if('try-error' %in% class(Reads_quality_try)){
      Reads_quality <- c(-1,-2)
    ...
    ##lastly the pattern of the mapping    
  ...
  cat("finish caluculating features for assessing the quality of mapping!")
  
  QC_result <- matrix(QC_result,ncol=14)
